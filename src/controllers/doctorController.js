...
let postDeletePatientForDoctor =   async ( req, res) => {
    try { 
        let infor = await doctorSevice.postDeletePatientForDoctor(req.query.doctorId, req.query.patientId, req.query.date);
        return res.status (200).json (
            infor
        )
